...
**Avance de la Fase 1:** HT-04 (3), HT-06 (5), HT-07 (3), HU-02 (5), HU-11 (2), HU-03 (8), HU-05a (3), HU-05b (2), HU-06.1 (5) y HU-08 (5) = 41 de 41 puntos (HU-11 y HU-08 con las salvedades de abajo). **HU-08 (5 pts):** consulta paginada implementada, con control de dueño y ruta protegida en el gateway; **salvedad:** el estado `en autenticación` aún no existe en el modelo (lo introduce HU-04, de otro integrante), por lo que hoy se listan `temporal` y `certificado`. **HU-03 completada:** la carga en `ms-documentos` y el consumidor de `ms-notificaciones` están implementados. HU-11 (2 pts): script mergeado y operador **MiFolio** registrado en GovCarpeta (ver `docs/OPERADOR_MINTIC.md`). **Verificación parcial:** el escenario del issue pide un `201` con el `operatorId` como texto plano, y esa respuesta del `POST` real no se pudo interpretar (el id se recuperó del directorio y la respuesta cruda no se conservó), así que ese detalle sigue sin confirmarse.
