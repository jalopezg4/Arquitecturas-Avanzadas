...
- **Registro (HU-01):** si GovCarpeta no responde o responde un 5xx distinto de 501 al registrar, el ciudadano queda `pendiente` (puede haber sido aceptado) y **no hay proceso de reconciliación**: ese documento responde `409` hasta que alguien lo resuelva a mano. Además `ms-documentos` **no consume** todavía `ciudadano.registrado` (la cola se llena sin consumidor; la carpeta se crea en la primera carga, así que no afecta el funcionamiento).
